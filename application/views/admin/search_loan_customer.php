      ...
      ?>
         ...
         <?php } ?>
             <?php if(@$customer_loan->loan_stat_date): ?>
                 <a href="<?= site_url('Admin/loan_return_statement/'.$customer_loan->customer_id) ?>" class="btn btn-info"><i class="icon-pencil">Loan Schedule</i></a>
             <?php endif ?>
...
<h7 class="title" id="defaultModalLabel"><?php echo $customer->f_name; ?> <?php echo $customer->m_name; ?> <?php echo $customer->l_name; ?><br>With Date:<?php if (@$customer_loan->loan_stat_date == TRUE) {
                        ...
                        <?php }elseif (@$customer_loan->loan_end_date == FALSE) {
                         ?>
                         YY-MM-DD
                         <?php } ?> <br> End Deposit Amount : <?php echo number_format(@$end_deposit->depost ?? 0); ?> <br>Deposit Time : <?php echo @$end_deposit->deposit_day; ?></h7>
</div>
                 <div class="text-center">
...
<input type="text" class="form-control" value="<?php if (@$total_deposit->total_Deposit > @$customer_loan->loan_int) {
                    ...
                    <?php echo number_format(@$customer_loan->loan_int); ?>
                     (<?php echo number_format(@$total_deposit->total_Deposit - @$customer_loan->loan_int); ?>)
                         <?php }else{ ?><?php echo number_format(@$total_deposit->total_Deposit ?? 0); ?>
                             <?php } ?>" readonly>     
</div>
...
<input type="text" class="form-control" value="<?php echo number_format($total_recovery->total_pending ?? 0); ?>.00" readonly style="color:red">
